...
def staking_vesting_allocation(params, substep, state_history, prev_state, **kwargs):
    ...
    agent_rewards = 0
    for agent in agents:
        agent_rewards += agents_staking_vesting_rewards[agent]
    assert abs(agent_rewards - staking_vesting_bucket_tokens) < 0.001, 'agent rewards '+str(agent_rewards)+' do not match macro rewards'+str(staking_vesting_bucket_tokens)
