...
def userAdoptionInput(sys_param, tav_return_dict):
    with st.expander("**User Adoption**"):
        ...
        if adoption_style == 'Custom' or show_full_adoption_table:
            # Choose between product fixed product growth model or an incentive based product growth model
            growth_model_choices = ['Fixed', 'Incentive Based']
            growth_model = st.radio('Adoption Growth Model:',tuple(growth_model_choices), index = growth_model_choices.index('Fixed'), help="Pick the type of User Adoption growth model you prefer").lower() # Review if relative selection for index param is better (see how done in agent section)

            if growth_model == 'fixed': #growth_model_choices[0]
            ...
            elif growth_model == "incentive based": 
                # incentivization_rev_share = float(sys_param['incentivisation_rev_share'][0])
                if tav_return_dict['incentivisation_allocation'] or tav_return_dict['airdrop_allocation'] > 0.0: # or incentivisation_rev_share > 0.0:
        ...
        switch = st.toggle('Activate', value = False, help="In this section, you can define the details regarding how you want to share revenues with other parties in your token economy. These include stakers, service providers, and incentive receivers. If no revenue share is selected, all the revenues are allocated to the business entity.") 
        ...
        if switch:
            ...
            business_rev_share = 100 # If no revenue share is selected, 100% of the revenue are set to go to the business.
            staker_rev_share = 0 
            service_provider_rev_share = 0
            incentivisation_rev_share = 0
            rev_share_sum = 100
            staker_rev_share_buyback = 0
            incentivisation_rev_share_buyback = 0
            user_adoption_target = 0
